...
void splitString(String str, char delimiter, String result[]) {
  ...
  char *ptr = strtok(str_array, &delimiter);
...
String getParsingString(String dest) { // fint array text
  String target_text = "gameIds";
  String result = "";

  int pos = dest.indexOf(target_text);
  int start_pos = dest.indexOf("[", pos);
  int end_pos = dest.indexOf("]", start_pos);

  result = dest.substring(start_pos + 1, end_pos);
...
String* split(String input, char delimiter) {
  ...
  for (size_t i = 0;i < input.length(); i++) {
    if (input.charAt(i) == delimiter) {
      result[array++] = input.substring(start, i);
      start = i + 1;
    }
  }

  result[array] = input.substring(start);

  return result;
}

String* parseString(String dest) {
  String* splited = split(dest, ',');
